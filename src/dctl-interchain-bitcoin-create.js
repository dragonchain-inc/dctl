...
program
  ...
  .option('-t, --testnet', '(optional) Sets this interchain to use the bitcoin testnet (defaults to mainnet). This value is automatically derived if providing private key as a WIF')
  ...
  .option('-c, --rpc-authorization [rpcAuthorization]', '(optional) The base64-encoded username:password for a secured rpc node. For example, user: a pass: b would be \'YTpi\' (base64("a:b"))')
...
util.wrapper(program, async client => {
  ...
  const response = await client.createBitcoinInterchain(util.removeUndefined({
    name,
    privateKey,
    rpcAddress,
    rpcAuthorization,
